...
const sections: Section[] = [
  {
    ...
    items: [
      ...
      {
        q: "ログイン方法は？",
        a: "メールアドレスを入力して「ログインリンクを送る」をクリックするだけです。パスワードは不要です。届いたメールのリンクをクリックするとログインできます。初めての方も同じ手順でアカウントが自動作成されます。",
      },
      {
        q: "最初にやることは？",
        a: "① メールアドレスでログイン → ② ヘッダーの「Googleカレンダー連携」でカレンダーを接続 → ③ 設定タブでAnthropicのAPIキーを登録 → ④ タスクを追加して研究を始めましょう。",
      },
      {
        q: "APIキーはどこで登録する？",
  ...
  },
  {
    ...
    items: [
      {
        q: "タスクタブには何がある？",
        a: "タスクタブは「研究タスク」と「Todoリスト」の2つのサブタブに分かれています。研究タスクは実験・論文などの研究活動を管理し、TodoリストはシンプルなチェックリストをSupabaseに保存・同期します。",
      },
      {
        q: "タスクを追加するには？",
      ...
      {
        q: "Todoリストの使い方は？",
        a: "タスクタブの「Todoリスト」サブタブを開き、テキストを入力してEnterキーまたは「追加」ボタンで登録します。チェックボタンで完了・未完了を切り替え、ホバーするとゴミ箱アイコンで削除できます。「完了済みをすべて削除」で一括削除も可能です。",
      },
      {
        q: "タスクをカレンダーに追加するには？",
